        ...
        <div className="row" style={{ marginTop: 12 }}>
          ...
          {[0, 3, 4, 4.5].map(r => (
            <button key={r} className="btn-sm" onClick={() => setF({ ...f, minRating: r })}
              style={f.minRating === r ? { borderColor: "var(--ink)", background: "var(--leaf2)" } : null}>{r === 0 ? "Any" : r + "★"}</button>
          ))}
          ...
          {shown.map(l => (
            <button className="listing" key={l.id} onClick={() => onOpen(l.id)}>
              <div className="photo" style={{ background: `linear-gradient(to bottom right, #FFF, ${TINT[l.cat]}40)` }}>
                ...
                <span className="mono" style={{ fontSize: 10, color: "var(--mute)", display: 'inline-flex', alignItems: 'center', gap: 4, fontWeight: 700 }}>{l.photos} photos{l.voice ? <><span style={{ margin: "0 4px" }}>·</span> {l.voice}s <Mic size="1em" /></> : ""}</span>
              </div>
